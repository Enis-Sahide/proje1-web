import { generateAstrologyChart, calculateDraconicChart, calculateHarmonicChart, calculateTransitAspects, generateTransitChart } from '../src/features/astrology/engine/AstrologyEngine';
...
const FIXED_STARS_2000 = [
  { name: 'Sirius', longitude: 104.08 },
  { name: 'Regulus', longitude: 149.83 },
  { name: 'Antares', longitude: 249.77 },
  { name: 'Aldebaran', longitude: 69.78 },
  { name: 'Spica', longitude: 203.83 }
];
...
async function runTest() {
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    counts[activeLevel as 1 | 2 | 3 | 4]++;
    currentMoment.add(1, 'day');
  }

  console.log('--- 2026 YILI 4 HARİTAYA AYRI TRANSİT HESAPLAMA SONUÇLARI ---');
  console.log(`1. Harita (Assiah): ${counts[1]} gün`);
  console.log(`2. Harita (Yetzirah): ${counts[2]} gün`);
  console.log(`3. Harita (Beriyah): ${counts[3]} gün`);
  console.log(`4. Harita (Atzilut): ${counts[4]} gün`);
}
