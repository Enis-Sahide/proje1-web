...
function GuestCheckoutForm() {
  ...
  const amount = analysisType === 'kabbalah' || analysisType === 'human-design' ? 500 : 50;
  ...
  // Listen for iFrame payment success message
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === '7LAYERS_PAYMENT_SUCCESS') {
        const token = event.data.token;
        setSuccessToken(token);
        setSuccess(true);
        setTimeout(() => {
          router.push(`/checkout/success?token=${encodeURIComponent(token)}`);
        }, 1500);
      }
    };
  ...
  // Proceed to iFrame payment step
  const handleProceedToPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIframeUrl(checkoutData.iframeUrl || `/checkout/iframe?orderId=${checkoutData.orderId}`);
      setStep('payment');
    } catch (err: any) {
  ...
  if (success) {
    return (
      <div className="max-w-md w-full bg-black/90 backdrop-blur-xl border border-green-500/20 p-8 rounded-3xl text-center shadow-[0_0_50px_rgba(34,197,94,0.2)] flex flex-col items-center">
        <div className="w-16 h-16 rounded-full bg-green-500/10 border border-green-500/30 flex items-center justify-center mb-6 shadow-inner">
        ...
        <p className="text-mystic-text-muted text-sm mb-4">
          Ödemeniz güvenle onaylandı. PDF indirme bağlantınız hazırlanıyor ve e-postanıza iletiliyor...
        </p>
    ...
    <div className="max-w-4xl w-full bg-black/80 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
      ...
      <div className="flex-1 p-6 md:p-8 flex flex-col justify-center">
        {step === 'info' ? (
          ...
          /* Step 2: Embedded iFrame Payment Gateway Container */
          <div className="space-y-3 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between border-b border-white/10 pb-2">
              <div>
                <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
                  <Lock size={14} className="text-[#D4AF37]" />
                  2. Adım: Güvenli Ödeme Ekranı
                </h3>
                <p className="text-[10px] text-white/40">Sistem içerisindeki güvenli geçit</p>
              </div>
              <button
                type="button"
            ...
            <div className="w-full rounded-2xl overflow-hidden border border-[#D4AF37]/20 shadow-2xl bg-[#05050A]">
              <iframe
                src={iframeUrl}
                title="Güvenli Ödeme Formu"
                className="w-full h-[510px] border-0"
                allow="payment"
              />
            </div>
          </div>
