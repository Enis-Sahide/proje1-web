...
export async function GET() {
  try {
    return NextResponse.json({
      success: true,
      questions: CHAKRA_TEST_QUESTIONS
    });
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    answers.forEach(ans => {
      const q = CHAKRA_TEST_QUESTIONS.find(item => item.id === ans.questionId);
      if (q) {
        chakraScores[q.chakraId] += ans.score;
      }
    });

    // Her çakranın yüzdesini hesapla
    // Web'deki getPercentage: Math.round(((score - 3) / 6) * 100)
    // Tıkanıklık yüzdesi (blockage) = 100 - getPercentage(score)
    ...
    // En çok tıkalı olandan (en yüksek yüzdenin olduğu) en aza doğru sırala
